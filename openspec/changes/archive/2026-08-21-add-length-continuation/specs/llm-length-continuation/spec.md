# llm-length-continuation 变更规格

## ADDED Requirements

### Requirement: length 续写触发与预算扩容

启用续写能力（`openai.length_continue` 任一字段非零）时，受覆盖调用点的一个 LLM 回合响应 `finish_reason == "length"` SHALL 触发续写：保留已输出内容，向该回合上下文（`round`）追加续写脚手架后重发请求，不终止循环。第 N 次尝试（0 基）请求的 `MaxTokens` SHALL 为该 agent 配置的 `max_tokens + N × expand_step`（thinking 家族映射到 `max_completion_tokens` 的现有规则不变）。续写尝试 SHALL NOT 消耗该 agent 的 `max_rounds` 轮数。能力未启用时 SHALL 保持现状行为逐字节不变（`length` 静默终止，`shouldDispatchToolCalls` 对 length+tool_calls 的 WARN + terminal 处理不变）。
...
- **WHEN** 一轮发生 2 次续写（共 3 次尝试），agent 配置 `max_tokens: 8192`、`expand_step: 8192`
...
- **WHEN** `openai.length_continue` 未配置或全零，某回合返回 `finish_reason=length`
...
系统 SHALL 支持全局配置块 `openai.length_continue`，含 `expand_step`（int，缺省 8192）与 `max_retries`（int，缺省 3）。块未配置或全零 SHALL 禁用续写；任一字段非零即启用，未设置的兄弟字段取缺省。负值或非整数值 SHALL 使配置加载失败。

#### Scenario: 缺省禁用

- **WHEN** `openai.length_continue` 未配置
- **THEN** 续写能力禁用，`length` 行为与改动前一致

#### Scenario: 部分配置补默认

- **WHEN** 配置 `length_continue: { expand_step: 4096 }`
- **THEN** 能力启用且 `max_retries` 取缺省 3

#### Scenario: 非法值拒绝加载

- **WHEN** `expand_step` 或 `max_retries` 为负数
- **THEN** 配置加载失败，服务不启动
