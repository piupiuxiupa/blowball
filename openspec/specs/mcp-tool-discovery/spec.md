...
定义 `GET /api/v1/mcp/tools` 端点只返回 MCP 来源（operator 全局 proxy 工具 + 调用者 per-user 缓存工具）的契约，并约束其仅基于缓存读取、不发起任何 MCP 连接，以保证响应延迟与配置的 server 数量无关、且用户隔离仅源自 JWT 推导的工作空间根。
