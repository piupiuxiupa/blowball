# xizhi-find-files Specification

## Purpose

TBD — 定义 `xizhi_find` Xizhi 工作空间条目查找工具：在调用者工作空间内按 RE2 正则匹配条目名（basename）递归查找文件与目录，支持类型过滤、深度限制、忽略大小写、隐藏条目开关与分页，替换原 `xizhi_glob_files`（glob 语法弱于 regex、无类型信息、无分页）；双引擎（系统 fd/fdfind shell out 与纯 Go filepath.WalkDir）保证逐字段一致的结果。
