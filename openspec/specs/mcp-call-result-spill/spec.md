# mcp-call-result-spill Specification

## Purpose

定义 `mcp_call` 返回结果的 token 估算、阈值判定、超大结果自动落盘（位置/命名/内容格式）、小信封返回形状、主动 `output_path` 参数、落盘失败降级，以及配置项 `tools.user_mcp.max_inline_result_tokens`。目标是让超大 MCP 结果不再原样进入模型上下文、SSE 事件与持久化行，而是落盘为 workspace 内文件并返回引导读回的小信封（读回侧复用既有 `xizhi_read_file`/`xizhi_grep`，零新代码）。
