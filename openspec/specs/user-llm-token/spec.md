# user-llm-token Specification

## Purpose

定义 per-user LLM 网关 token（`user_llm_credentials` 表）的存储、管理 API、按用户的客户端解析与全局回退语义、全部用户可归属 LLM 调用点的凭证口径、配置错误时的显式失败，以及 token 保密不变量。用户 token 仅覆盖 `openai.api_key` 一个维度：`openai.base_url`、模型目录与所有模型元数据保持部署级全局配置；未配置 token 的用户回退全局 `openai.api_key`，行为与引入本能力之前一致。
