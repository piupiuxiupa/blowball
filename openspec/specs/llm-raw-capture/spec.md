# llm-raw-capture Specification

## Purpose

将每次 LLM 调用的原始请求/逐帧/响应/错误 payload 经 Redis write-behind 缓冲批量写入 MySQL `llm_raw_log` 表，作为默认常开、不阻塞主路径的观测旁路：留存实发全量请求 JSON、网关原样 SSE 帧、缝合响应与错误 body 及上下文关联列（trace/session/user/agent），用于网关/模型行为问题的事后排查与回放；表不挂会话外键、永不自动清理。
...
- **THEN** the call's final row is `kind=error` with `http_status=0`, a raw body naming the idle timeout plus frames received and model, ordered after the last `kind=chunk` row of the same `call_id`
