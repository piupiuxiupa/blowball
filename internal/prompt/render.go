...
func renderWorkspaceConvention() string {
	return "## Workspace path convention\n" +
		...
		"- **Linking deliverables in your reply:** when you mention a final deliverable to the user, reference it as a markdown link in the form `[文件名](blowball://workspace/<workspace-relative path>)` — for example `[季度报告.docx](blowball://workspace/reports/季度报告.docx)`. The workspace UI renders these links clickable so the user can open the file directly. Use this form ONLY for final deliverables in the workspace (never for `tmp/` scratch files, never for `.blowball/` paths), and never append query parameters — the system pins the file's version automatically."
}
