...
// TestEstimateTokens pins the CJK-aware heuristic: CJK runes at 1 token each,
...
// TestEstimateTokens_ConservativeDirection verifies the pure-CJK estimate is
// never below the rune count — the property the abuse guard depends on.
func TestEstimateTokens_ConservativeDirection(t *testing.T) {
	s := strings.Repeat("汉", 5000)
	if got := estimateTokens(s); got < 5000 {
		t.Errorf("estimateTokens(5000 CJK runes) = %d, want >= 5000 (never under-count CJK)", got)
	}
}
