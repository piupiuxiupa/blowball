...
- `Agent` interface: `Run(ctx, messages, hub)` returns content, usage, and a per-agent `*TurnBreakdown` (Confucius only; leaf agents return nil and the parent aggregates). The breakdown carries `by_agent` (per-agent usage keyed by display name, always including "Confucius") plus turn meta (`parallel` — whether any assistant round dispatched ≥2 tool_calls; `sub_agent_invocations` — ordered invoke_* names).
...
`internal/stream/event.go` defines `StreamEvent` and event types: `agent_start`, `token`, `reasoning`, `tool_call`, `tool_result`, `agent_end`, `agent_error`, `done`. `reasoning` carries thinking-mode chunks (emitted when an agent runs with `thinking: true`). `message` is a sentinel type used only for persisted user-message rows; it is never emitted over SSE. The terminal `done` event carries `Meta.usage` in the authoritative nested shape `{total:{prompt_tokens,completion_tokens,total_tokens,reasoning_tokens?}, by_agent:{<agent>:{...}}, meta:{sub_agent_invocations:[...], parallel:bool}}` — the legacy flat top-level token fields were moved under `total`; frontend consumers reading `usage.total_tokens` must migrate to `usage.total.total_tokens`.
