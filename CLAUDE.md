...
Every registry-tool result is rendered into a uniform status envelope before being fed back to the model as the `role="tool"` message body: `{"status":0,"result":<JSON-encoded tool return value>}` on success and `{"status":1,"error":"<message>"}` on failure. The envelope is produced by one shared renderer (`renderToolResult` in `internal/agent/confucius.go`) invoked from all three agents' registry-tool dispatch sites (Confucius/Chongzhi/Liang); `[]byte` and `string` returns are normalized to text before encoding (not base64), and `nil` renders as `result:null`. It is a model-facing signal only — the `agent_error` SSE event is still emitted independently on failure for the frontend. Sub-agent (`invoke_*`) results bypass the envelope: their content is the sub-agent's output/error text verbatim.
